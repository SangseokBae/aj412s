...
cat("  KoreaMacro<-read_excel('Data41_Time_Series_data3.xlsx', skip=1) ", '\n')
cat("      ", '\n')
cat("  또는, KoreaMacro로 저장된 내장데이터를 사용한다. 동일한 자료가 KoreaMacro내장데이터로 저장되어 있다. ", '\n')
cat("  df<-KoreaMacro ; colnames(df)<-df[1,] ; df<-df[-1,] ", '\n')
cat("  index<-c(3:7); df[,index]<-lapply(df[,index], as.numeric)  ", '\n')
cat("  df<-mkdate_form(df, 2)  ", '\n')
cat("  df$year<-substring(df$yearmonth, 1,4) ", '\n')
cat("  df$month<-substring(df$yearmonth, 6,7) ", '\n')

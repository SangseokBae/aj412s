...
cat("  df2 <- df[ complete.cases(df$car_conv, df$car_satprice, df$age), ]  ", '\n')
...
cat("## 문자 레코드를 숫자로 바꾸기 -------- ", '\n')
cat("  library(aj412s) ", '\n')
cat("  df$car_brand2<-c2n(df$car_brand) ", '\n')
cat("   ", '\n')
...
cat("   ", '\n')
cat("## 자료수 확인 ------- ", '\n')
cat("  sum(is.na(df$car_conv))  ", '\n')
cat("  sum(!is.na(df$car_conv))  ", '\n')
cat("  sum(df$car_brand=='Ford') ", '\n')
cat("   ", '\n')

invisible(readline(prompt="Press [enter] to continue"))
cat("   ", '\n')
cat("## xtabs를 사용해 막대그래프 그리기 ------- ", '\n')
cat("  xtabs(~df$gender+df$car_brand)  ", '\n')
cat("  barplot(xtabs(~df$gender+df$car_brand), beside=T, horiz=F, ylab='빈도수')  ", '\n')
cat("   ", '\n')
...
cat("   ", '\n')
cat("  df %>% group_by(브랜드)  ", '\n')
cat("        %>% summarize(편의성평균=mean(car_conv, na.rm=T), 표준편차=sd(car_conv, na.rm=T), 자료수=n())  ", '\n')
cat("   ", '\n')
cat("  df %>% select(car_brand, gender) ", '\n')
cat("   ", '\n')

...
cat("  mean(df$income, na.rm=T)  ", '\n')
cat("  sd(df$income, na.rm=T) ", '\n')
cat("  df%>%summarize(평균=mean(income), na.rm=T)   ", '\n')
cat("   ", '\n')
...
cat("   ", '\n')
cat("  merge(data1, data2, by='index') %>% merge(data3, by='index') %>% merge(data4, by='index') ", '\n')
cat("      ", '\n')
